...
async def list_fields(table_id: str, client_info: tuple = Depends(get_nocodb_client)):
    url, headers = client_info
    endpoint = f"{url}/api/v1/meta/tables/{table_id}/columns"
    async with httpx.AsyncClient() as client:
        # Depending on NocoDB version logic, try the meta fields api
        endpoint = f"{url}/api/v1/meta/tables/{table_id}/fields"
        res = await client.get(endpoint, headers=headers)
        
        # fallback just in case version differs
...
async def create_table(base_id: str, data: CreateTableSchema, client_info: tuple = Depends(get_nocodb_client)):
    ...
    async with httpx.AsyncClient() as client:
        ...
        for col in data.columns:
            ...
            if safe_col_name.lower() == "id":
                safe_col_name = "csv_id"
